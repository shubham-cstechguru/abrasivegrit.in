...
<div class="row mb-4">
	<div class="col-lg-3 px-0 mt-1" id="sidebar_data">
		<div class="sticky">
			<p class="product-range my-0">All Products</p>
			<!--Accordion wrapper-->
						...
						<div class="card-body card-in-list">
							<ul>@foreach($list->product as $list1)
								<li style="@if($list1->title == $slug->title) background: #aaa; @endif">
									<a href="{{ $list1->slug }}"><i class="rotate-icon">»</i> &nbsp;<p style="overflow: hidden; min-width: 5ch;  max-width: 25ch; text-overflow: ellipsis; white-space: nowrap;">{{ $list1->title }} </p></a>
								</li>
								@endforeach
							</ul>
						</div>
			...
			<div class="row">
				<div class="col-lg-4 col-12 mt-3">
					<div class="sticky">
						<div>
							@if($slug->image!='')
							<img class="lazy-load" src="{{url('imgs/loader-2.gif')}}" data-src="{{url('imgs/product/'.$slug->image)}}" alt="{{ $slug->title }}" onclick="@if(!empty($slug->product_img)) openModal() @endif" style="@if(!empty($slug->product_img)) cursor: pointer; @endif">
							@else
							<img class="lazy-load" src="{{url('imgs/loader-2.gif')}}" data-src="{{url('imgs/unavailable-image-300x225.jpg')}}" alt="{{ $list->title }}" onclick="@if(!empty($slug->product_img)) openModal() @endif" style="@if(!empty($slug->product_img)) cursor: pointer; @endif">
							@endif
						</div>
